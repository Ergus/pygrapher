...
re_result: Pattern[str] = re.compile(r"\S+//?(\S*?)\.json(\.\d+)")
re_external: Pattern[str] = re.compile(r"# Command:.*?-o " + re_result.pattern +".*?Total execution time: (\S+s)", re.DOTALL)
re_filename: Pattern[str] = re.compile(r"(\d+)_(\d)threads")


def translateStrToTime(pvalue : str) -> float:
    matches = re.match("([\d\.]+)(us|ms|s|m|h|d)", pvalue)
    if not matches:
...
class MyTable(dict):
    @staticmethod
    def processMultiple(jsonfiles: list[str], logfilename: str | None) -> Container:

        prefix: str = os.path.dirname(os.path.commonprefix(jsonfiles))

        matches = re.fullmatch(re_filename, prefix)
        if not matches:
            raise Exception(f"Filename prefix:{prefix} does not match regex")

        prefixdict = {"threads": int(matches[2]), "paths": int(matches[1])}
...
if __name__ == "__main__":
    prefix: str = os.path.dirname(os.path.commonprefix(sys.argv[1:]))

    data: MyTable = MyTable(sys.argv[1:], None)
